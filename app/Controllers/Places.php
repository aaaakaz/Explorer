...
class Places extends BaseController {
    ...
    public function detail(int $id): string {
        ...
        // Fetch and cache Unsplash photo if not stored yet
        if (empty($place['photo_url'])) {
            ...
            if ($key) {
                ...
                $ch  = curl_init($url);
                curl_setopt_array($ch, [CURLOPT_RETURNTRANSFER=>true,CURLOPT_TIMEOUT=>5,CURLOPT_SSL_VERIFYPEER=>false,CURLOPT_HTTPHEADER=>['Accept-Version: v1']]);
                $raw = curl_exec($ch); curl_close($ch);
                if ($raw) {
                    $data = json_decode($raw, true);
                    $photoUrl = $data['results'][0]['urls']['small'] ?? null;
                    if ($photoUrl) {
                        try { $this->placeModel->update($id, ['photo_url' => $photoUrl]); } catch (\Exception $e) {}
        ...
        if (session()->get('logged_in')) {
            try {
                $this->db->query(
                    "INSERT INTO recently_viewed (user_id, place_id) VALUES (?, ?) ON DUPLICATE KEY UPDATE viewed_at = CURRENT_TIMESTAMP",
